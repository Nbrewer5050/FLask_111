...
def get_db():
    ...
    if not db:
        db = g._datbase = sqlite3.connect(DATABASE)
    return db
...
def update(prod_id, fields: dict):
    field_string =", ".join("%s=\"%s\"" % (key, val) for key, val in fields.items())
    query = """
            UPDATE product
            SET %s
            WHERE id = ?
...
def create(name, price, category, description):
    ...
    query = """
            ...
            """
    cursor = get_db()
    last_row_id = cursor.execute(query, value_tuple).lasstrowid
    cursor.commit()
    return last_row_id
